...
def _account_filter_sql(
    ...
    if plan and plan not in {"all", "any"}:
        ...
        elif plan in {"half_price", "plus_half_price", "half_price_trial"}:
            half_price_expr = "lower(COALESCE(CAST(json_extract(payload, '$.plus_half_price_eligible') AS TEXT), ''))"
            where.append(f"{plan_expr} = ?")
...
def _decorate_account(row: dict) -> dict:
    ...
    if plan_status in {"queued", "running"}:
        try:
            ...
            out["plan_check_stale"] = True
    momo_status = out.get("momo_check_status")
    if momo_status in {"queued", "running"}:
        try:
            stamp_key = "momo_check_queued_at" if momo_status == "queued" else "momo_check_started_at"
            started_at = datetime.fromisoformat(str(out.get(stamp_key) or ""))
            if (datetime.now() - started_at).total_seconds() >= _MOMO_CHECK_STALE_SECONDS:
                out["momo_check_status"] = "failed"
                out["momo_check_error"] = "上次 MoMo 检测已超时，可重新检测"
        except (TypeError, ValueError):
            out["momo_check_status"] = "failed"
            out["momo_check_error"] = "上次 MoMo 检测状态异常，可重新检测"
    out["copy_line"] = _account_line(out)
    return out


def _account_matches_plan_filter(row: dict, plan_filter: str | None = None) -> bool:
    """账号套餐过滤：支持已开通、可试用、半价试用及不可试用 Plus。"""
...
def claim_account_momo_check(acc_id: int, trigger: str = "manual") -> bool:
    """Atomically reserve an account for one MoMo checkout capability probe."""
    with _LOCK:
        accounts = _load_accounts()
        row = next((r for r in accounts if int(r.get("id") or 0) == int(acc_id)), None)
        if row is None or row.get("momo_check_status") in {"queued", "running"}:
            return False
        now = _now()
        row.update({
            "momo_check_status": "queued", "momo_check_ok": False,
            "momo_check_trigger": str(trigger or "manual"),
            "momo_check_queued_at": now, "momo_check_started_at": None,
            "momo_check_completed_at": None, "momo_check_error": None,
            "updated_at": now,
        })
        _save_accounts(accounts)
        return True
...
def mark_account_momo_check_running(acc_id: int) -> bool:
    with _LOCK:
        ...
        if row is None or row.get("momo_check_status") not in {"queued", "running"}:
            return False
        row.update({"momo_check_status": "running", "momo_check_started_at": _now(), "momo_check_error": None, "updated_at": _now()})
        _save_accounts(accounts)
        return True


def update_account_momo_check(acc_id: int, result: dict | None = None) -> bool:
    """Persist only redacted MoMo checkout capability data for an account."""
    result = result or {}
    with _LOCK:
        accounts = _load_accounts()
        row = next((r for r in accounts if int(r.get("id") or 0) == int(acc_id)), None)
        ...
        now = _now()
        row.update({
            "momo_check_status": "success" if ok else "failed",
            "momo_check_ok": ok,
            "momo_check_completed_at": now,
            "momo_checked_at": result.get("checked_at") or now,
            "momo_check_http_status": result.get("http_status"),
            "momo_check_error": None if ok else str(result.get("error") or "MoMo 检测失败")[:600],
            "momo_decision": result.get("decision"),
            "momo_decision_text": result.get("decision_text"),
            "momo_available": result.get("has_momo"),
            "momo_actual_trial": result.get("actual_trial"),
            "momo_methods": result.get("methods") or [],
            "momo_payment_methods_known": result.get("payment_methods_known"),
            "momo_stripe_mode": result.get("stripe_mode"),
            "momo_check_network_route": result.get("network_route"),
            "momo_check_proxy_used": result.get("proxy_used"),
            "updated_at": now,
        })
        _save_accounts(accounts)
        return True
    ...
    with _LOCK:
        ...
        for row in accounts:
            if row.get("momo_check_status") not in {"queued", "running"}:
                continue
            row.update({"momo_check_status": "failed", "momo_check_ok": False, "momo_check_error": "WebUI 重启导致 MoMo 检测中断，请重新检测", "momo_check_completed_at": _now(), "updated_at": _now()})
            count += 1
        if count:
            _save_accounts(accounts)
        return count
...
def list_account_plan_check_statuses(
    ...
    fields = (
        ...
        "momo_check_status", "momo_check_ok", "momo_check_error", "momo_checked_at",
        "momo_check_http_status",
        "momo_decision", "momo_decision_text", "momo_available", "momo_actual_trial",
        "momo_methods", "momo_payment_methods_known", "momo_stripe_mode",
            ...
            [
                {
                    ...
                    "momo_check_http_status": row.get("momo_check_http_status"),
                    "momo_decision": row.get("momo_decision"),
                    "momo_available": row.get("momo_available"),
...
def update_account_security_material(
    acc_id: int,
    *,
    registration_password: str | None = None,
    access_token: str | None = None,
    twofa_network_traffic: dict | None = None,
    stage: str | None = None,
    error: str | None = None,
) -> bool:
    """Persist password/session material produced by the pre-2FA password flow."""
    with _LOCK:
        rows = _load_accounts()
        row = next((r for r in rows if int(r.get("id") or 0) == int(acc_id)), None)
        if row is None:
        ...
        if registration_password is not None:
            ...
            if not isinstance(extra, dict):
                extra = {}
            extra["registration_password"] = str(registration_password)
            row["extra_json"] = json.dumps(extra, ensure_ascii=False)
        if access_token:
            row["access_token"] = str(access_token)
        if twofa_network_traffic is not None:
            row["totp_network_traffic"] = dict(twofa_network_traffic)
        if stage is not None:
            row["totp_setup_stage"] = str(stage)
        row["totp_setup_error"] = error
        row["copy_line"] = _account_line(row)
        row["updated_at"] = _now()
        _save_accounts(rows)
        return True
...
def claim_next_outlook() -> dict | None:
    """从可用池随机原子领取一个 Outlook 账号并标记为 used。"""
    with _LOCK:
        rows = _load_outlook()
        registered = {
        ...
        for candidate in rows:
            ...
            if candidate.get("status") == "available" and candidate_email in registered:
                candidate["status"] = "used"
                candidate["used_at"] = candidate.get("used_at") or _now()
                candidate["registration_consumed"] = True
                candidate["registration_disposition"] = "already_registered"
                candidate["note"] = "自动标记已用: 邮箱已存在于注册账号表"
                changed = True
