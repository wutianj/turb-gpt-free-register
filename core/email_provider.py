...
_job_source = threading.local()


def set_job_email_source(source: str | None) -> None:
    _job_source.value = source


def clear_job_email_source() -> None:
    if hasattr(_job_source, "value"):
        del _job_source.value
...
def parse_email_sources(value=None) -> list[str]:
    ...
    if value is None:
        ...
        value = getattr(_job_source, "value", None) or _email_cfg.EMAIL_SOURCE
...
REGISTRATION_EMAIL_CONSUMED_MARKER = "registration_email_consumed"


def registration_failure_consumes_email(error: object) -> bool:
    """Whether a failed registration email must stay in the used pool."""
    text = str(error or "")
    if not text:
        return False
    return (
        REGISTRATION_EMAIL_CONSUMED_MARKER in text
        or (
        ...
        or "existing_account_mfa_challenge" in text
        or "邮箱已注册且启用了 2FA" in text
        or "找不到 OTP 输入框" in text
        or "邮箱验证码连续错误/过期" in text
        or "找不到可点击的重新发送验证码按钮" in text
        or "等待/填写资料页超时" in text
        or ("OTP 超时" in text and "等待 " in text)
    )


def mark_registration_email_used(email: str, reason: str) -> str:
    """Keep a submitted or already-registered mailbox out of future claims."""
    return release_email(
        email,
        status="used",
        note=f"自动标记已用: {str(reason or '')[:180]}",
    )
