...
# ---------- Roxy/Cloak 浏览器省流量模式 ----------
# 默认开启。开启后只拦截可选的图片/媒体/字体/清单，以及下面明确列出的统计/第三方 URL；
...
# 默认拦截 image、media、font、manifest；也可配置 stylesheet 等资源；Roxy 还会通过 Chromium 启动参数关闭图片加载，
# 遇到页面布局或验证码异常时可关闭模式。
BROWSER_DATA_SAVER_BLOCKED_RESOURCE_TYPES: list[str] = ["image", "media", "font", "manifest"]
