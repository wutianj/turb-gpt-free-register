...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/browser/delete') {
      const ids = Array.isArray(body.dirIds)
        ? body.dirIds
        : [body.dirId ?? body.profileId ?? body.profile_id];
      const validIds = ids.filter((id) => id !== undefined && id !== null && String(id).trim());
      if (!validIds.length) return err(res, 'dirId is required');
      ...
      return ok(res, { deleted: validIds.length });
    }
