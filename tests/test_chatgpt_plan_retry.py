...
class _NetworkThenSuccessSession(_PlanSession):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.responses = [_Response(200, data={
            "accounts": {
                "acc-1": {
                    "account": {"account_id": "acc-1", "plan_type": "free"},
                    "entitlement": {},
    ...
    def get(self, url, headers=None, **_kwargs):
        self.requests.append((url, dict(headers or {})))
        if len(type(self).created) == 1 and len(self.requests) == 1:
...
class ChatgptPlanRetryTests(unittest.TestCase):
    ...
    def test_network_error_rotates_implicit_proxy_before_retry(self):
        claims = {
            "payload": {}, "email": "one@example.com", "account_id": "acc-1",
            "token_expired": False, "claim_plan_type": "free",
        }
        routes = [
            {"proxy": "socks5h://proxy-one:1080", "proxy_mode": "auto", "network_route": "proxy", "proxy_used": "socks5h://proxy-one:1080", "proxy_fallback_reason": None},
            {"proxy": "socks5h://proxy-two:1080", "proxy_mode": "auto", "network_route": "proxy", "proxy_used": "socks5h://proxy-two:1080", "proxy_fallback_reason": None},
        ]
        _NetworkThenSuccessSession.created = []
        with patch.object(plan, "BrowserSession", _NetworkThenSuccessSession), patch.object(
            plan, "token_claims", return_value=claims
        ), patch.object(plan, "resolve_plan_check_route", side_effect=routes), patch.object(
            plan, "_warm_plan_session"
        ), patch.object(plan.time, "sleep"):
            result = plan.check_account_plan("token", max_attempts=2, retry_delay=0)

        self.assertTrue(result["ok"])
        self.assertEqual(len(_NetworkThenSuccessSession.created), 2)
        self.assertEqual(result["proxy_used"], "socks5h://proxy-two:1080")

    def test_network_failure_uses_roxy_browser_fallback_after_last_attempt(self):
        claims = {
            "payload": {}, "email": "one@example.com", "account_id": "acc-1",
            "token_expired": False, "claim_plan_type": "free",
        }
        route = {"proxy": "socks5h://proxy:1080", "proxy_mode": "auto", "network_route": "proxy", "proxy_used": "socks5h://proxy:1080", "proxy_fallback_reason": None}
        browser_body = '{"accounts":{"acc-1":{"account":{"account_id":"acc-1","plan_type":"free"},"entitlement":{}}}}'
        _NetworkThenSuccessSession.created = []
        with patch.object(plan, "BrowserSession", _NetworkThenSuccessSession), patch.object(
            plan, "token_claims", return_value=claims
        ), patch.object(plan, "resolve_plan_check_route", return_value=route), patch.object(
            plan, "_warm_plan_session"
        ), patch.object(plan, "_request_plan_with_roxy_browser", return_value=(200, browser_body)) as fallback, patch.object(plan.time, "sleep"):
            result = plan.check_account_plan("token", max_attempts=1, retry_delay=0)

        self.assertTrue(result["ok"])
        self.assertEqual(result["network_route"], "roxy_browser_fallback")
